...
Last verified against codebase: 2026-06-22, commit `2360070` (origin/main + local Team ID/preinstall fix).
...
- **Calendar/comms:** Google Calendar is genuinely integrated (OAuth + sync). **Garmin and Twilio are listed in the in-app connections UI but have no backend integration** — they're UI placeholders that imply a working connection where none exists; fix or relabel before launch.
...
- **Payments:** Not started. No Helcim, no paywall, no subscription code, no referral-on-decline flow anywhere in the repo.
...
5. **Sign-up pricing mechanic** ($20/mo → decline → referral offer: 1 month free for 3 referrals). **Status: not started.** No paywall, pricing screen, or referral flow exists.
...
- **Near-term:** a fast non-voice check-in path (typed/tap) that captures maximum cross-domain data in minimal time, for voice-cost control and users who won't talk to the app. (Not yet confirmed in code as of this pass — verify before assuming it's started.)
